...
export default function LoginModal() {
  ...
  const { setCookie, setProfile, cookie, isInitialLoading } = useAppStore();
  ...
  return (
    <AnimatePresence>
      <motion.div 
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-50 flex items-center justify-center bg-[#f5f5f7] dark:bg-black"
      >
          ...
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-black/5 mb-8 relative group w-48 h-48 flex items-center justify-center overflow-hidden">
            ...
            {status === 800 && !cookie && (
              <div className="absolute inset-0 bg-white/80 flex items-center justify-center backdrop-blur-sm rounded-2xl">
                <button 
                  onClick={initQrCode}
                  className="bg-[#0071e3] text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-[#0077ED] transition-colors shadow-sm"
                >
                  刷新二维码
                </button>
              </div>
